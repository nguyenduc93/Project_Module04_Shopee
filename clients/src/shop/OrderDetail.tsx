...
const OrderDetail = () => {
  ...
  const getOrder = async () => {
    try {
      ...
      const groupedData = data.data.message.reduce(
        (acc: { [orderId: number]: Product[] }, cart: Product) => {
          if (!acc[cart.orderId]) {
            acc[cart.orderId] = [];
          }
  ...
  const handleUpdate = async (id: number) => {
    ...
    try {
      let data = await axios.put(`http://localhost:5000/api/v1/order/${id}`, {
        statusOrder,
      });
          ...
          {Object.keys(groupedOrders).map((orderId) => (
            <div className="order_table1" key={orderId}>
              <div className="order_name">
                <Avatar
                  src={groupedOrders[Number(orderId)][0].avatarURL}
                  className="avatarrr"
                  size={35}
                >
                  ĐN
                </Avatar>
                <p>{groupedOrders[Number(orderId)][0].userName}</p>
                <MessageIcon style={{ color: "red", cursor: "pointer" }} />
              </div>

              <div className="order_table2">
                <div className="name_order">
                  {groupedOrders[Number(orderId)].map((order: Product) => (
                    <div className="name_order1" key={order.productId}>
                      <img src={order.image} alt="" width={70} />
                      <p>{order.productName}</p>
                    </div>
                  ))}
                </div>
                <div className="sum_order">
                  {formatCurrency(groupedOrders[Number(orderId)][0].totalPrice)}
                </div>
                <div className="status_order">
                  {groupedOrders[Number(orderId)][0].statusOrder ===
                  "pending" ? (
                    <button
                      style={{
                        color: "rgb(238,77,45)",
                ...
                >
                  ...
                  <p
                    onClick={() => showModal(Number(orderId))}
                    style={{ padding: 0 }}
                  >
                    Xem chi tiết
                  </p>
                        ...
                        <table className="table table_table">
                          <tbody>
                            ...
                            <tr>
                              ...
                              <td className="label1">{currentOrder.address}</td>
                            </tr>
                          </tbody>
                        </table>
                      </>
