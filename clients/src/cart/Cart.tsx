...
const Cart = () => {
  ...
  const handleDelete = async (id: number) => {
    try {
      let response = await axios.delete(
        `http://localhost:5000/api/v1/cart/${id}/${flagUser.userId}`
      );
  ...
  const handlePay = async () => {
    ...
    try {
      ...
      for (let i = 0; i < carts.length; i++) {
        ...
        orderIds.push(orderResponse.data.orderId);
      }
      ...
      try {
        for (let i = 0; i < carts.length; i++) {
          let cart = carts[i];
          await axios.post(`http://localhost:5000/api/v1/orderDetail`, {
  ...
  const updateProductQuantitySold = async (
    ...
    try {
      await axios.put(
        `http://localhost:5000/api/v1/products/buy/${productId}`,
        {
